-- not my code just used off of someone else that left the script scene here
-- Fixes Include Q not working after first initial combo "No more pressing F5 to reload"
-- R2 not being used when target is killable.
-- Fast combo always used for most dmg impact
-- fixed draw total dmg as this was also bugged
-- fixed Jungle Clear errors as well
...
function BaseStrucutre:Combo(postAttack)
    ...
    if (not isFastCombo or not IsInAARange(Target)) and CastE and spells.E:IsReady() and Player:Distance(Target) < spells.E.Range + Player.AttackRange - 50 then
        if spells.R:IsReady() and IsR1() and CastR then
            ...
            return
        elseif spells.R:IsReady() and IsR2() and CastR2 and rDmg >
            Target.Health  then
    ...
    if CastQ and spells.Q:IsReady() then
        if Player:IsFacing(Target, 30) and Player:Distance(Target) < spells.Q.Range +
